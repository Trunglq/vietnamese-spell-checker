...
"""
Test Categorized Vietnamese Spell Checker
"""

from categorized_spell_checker import categorized_spell_checker
import time

def test_categorized_spell_checker():
    """Test phiên bản categorized"""
    
    print("📊 Test Categorized Vietnamese Spell Checker")
    print("=" * 60)
    ...
    for i, text in enumerate(test_cases, 1):
        ...
        if result['error_count'] > 0:
            ...
            # Hiển thị lỗi theo category
            categories = result.get('error_categories', {})
            for category, count in categories.items():
                print(f"   📂 {category}: {count} lỗi")
            
            print("🔧 Chi tiết từng lỗi:")
            for error in result['errors']:
                category = error.get('category', 'unknown')
                print(f"   - '{error['word']}' → '{error['corrected']}' ({category})")
        else:
...
if __name__ == '__main__':
    test_categorized_spell_checker() 