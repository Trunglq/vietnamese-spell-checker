...
"""
Hybrid Vietnamese Spell Checker
Kết hợp ưu điểm của Advanced và Smart versions
"""
...
class HybridVietnameseSpellChecker:
    """Phiên bản hybrid của Vietnamese Spell Checker"""
    
    def __init__(self):
        self.vietnamese_dict = vietnamese_dict
        self.error_patterns = self._load_comprehensive_patterns()
        self.context_rules = self._load_context_rules()
    
    def _load_comprehensive_patterns(self) -> Dict[str, str]:
        """Tải các pattern lỗi toàn diện"""
        return {
            # Lỗi dấu thanh - sửa tất cả
            r'\bcôn\b': 'công',
            r'\bkin\b': 'kính',
            r'\btoi\b': 'tôi',
            r'\bdinh\b': 'định',
    ...
    def check_text(self, text: str) -> Dict:
        """Kiểm tra chính tả với thuật toán hybrid"""
        try:
            # Chuẩn hóa văn bản
            ...
            for i, word in enumerate(words):
                ...
                if not self._is_correct_word(clean_word):
                    suggestions = self._get_hybrid_suggestions(clean_word, words, i)
                    if suggestions:
                        errors.append({
                            'word': word,
            ...
            # Áp dụng corrections
            corrected_text = self._apply_hybrid_corrections(normalized_text, errors)
    ...
    def _get_hybrid_suggestions(self, word: str, all_words: List[str], position: int) -> List[str]:
        """Lấy gợi ý hybrid cho từ"""
        suggestions = []
        ...
        # Validate suggestions (ít strict hơn)
        validated_suggestions = []
        for suggestion in suggestions:
            if self._validate_hybrid_suggestion(word, suggestion, all_words, position):
                validated_suggestions.append(suggestion)
        
        return validated_suggestions[:5]  # Trả về tối đa 5 gợi ý
    ...
    def _validate_hybrid_suggestion(self, original: str, suggestion: str, all_words: List[str], position: int) -> bool:
        """Validate gợi ý hybrid (ít strict hơn)"""
        # Không sửa thành từ quá dài
        if len(suggestion) > len(original) + 3:
            return False
        ...
        # Kiểm tra trong từ điển hoặc common errors
        if (self.vietnamese_dict.is_correct_word(suggestion) or 
            suggestion.lower() in self.vietnamese_dict.common_errors):
            return True
        
        return False
    
    def _apply_hybrid_corrections(self, text: str, errors: List[Dict]) -> str:
        """Áp dụng corrections hybrid"""
        corrected_text = text
...
# Tạo instance global
hybrid_spell_checker = HybridVietnameseSpellChecker() 